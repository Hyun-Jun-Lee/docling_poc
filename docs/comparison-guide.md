...
기존 비교 실행과 보고서 재생성 명령은 `report.md`도 UTF-8로 저장한다. 추가 옵션이나 재추출은 필요 없다. 용량을 줄이기 위해 도구별 Markdown·JSON 원문 비교는 제외한다. HTML과 동일한 측정값·실행 상태·기능별 발췌를 사용하며, 측정 결과와 운영 확장성은 표로, 기능별 발췌는 소제목과 코드 블록으로 표시한다. 발췌의 코드 울타리와 HTML은 원문으로 보존한다. 전체 원문과 버튼·스타일 전환은 HTML에서 확인한다. 원본 파일은 변경하지 않는다.

`--standalone`을 사용해도 Markdown은 원래 결과 폴더의 `report.md`에 저장된다. Markdown에도 문서의 일부 내용이 포함되므로 공유 범위에 주의한다. 기존 `report.md`에 적용하려면 보고서를 재생성한다.
