...
| 파일·폴더 | 내용 |
|---|---|
| `report.md` | 측정 결과, 실행 상태, 기능별 추출 정보, 운영 확장성 |
...
Markdown 보고서에서는 용량을 줄이기 위해 도구별 Markdown·JSON 원문 비교를 제외합니다. 측정 결과·실행 상태·기능별 추출 정보 발췌·운영 확장성은 유지합니다. 기능별 발췌는 소제목과 코드 블록으로 표시합니다. 전체 원문은 HTML 또는 해당 회차의 원본 파일에서 확인할 수 있습니다. `--standalone`은 추가 HTML 공유본만 지정하며 `report.md`는 원래 결과 폴더에 저장합니다.
