...
| 형식 | 구조를 얻는 주된 근거 | 현재 예시에서 보이는 결과 | 변환 후 보완의 초점 |
| --- | --- | --- | --- |
| PDF | 레이아웃 분석, 내장 텍스트, OCR, 표·그림 인식 | `section_header`, `list_item`, `page_header/footer`, bbox가 풍부함 | Docling 네이티브 라벨을 우선 사용하고, 저품질 OCR·읽기 순서·그림 영역을 검수/보강 |
| DOCX | OOXML의 스타일·개요 수준·번호 매기기·표·인라인 객체 | 이 예시에서는 다수 항목이 `text`; `1. 모집개요`, `가. 모집대상`도 별도 제목 라벨이 없음 | 번호 패턴·실제 formatting·인접 관계 등 문서별 보강 정책을 추가하되 근거를 기록 |
| PPTX | 제목 자리표시자, 텍스트 도형, 글머리표/번호, 슬라이드·그룹 구조 | 현재 결과에 `chapter`, `title`, `list_item`, `paragraph`, `picture`, `table`이 공존 | 슬라이드 단위와 자리표시자 구조를 우선 보존하고, 일반 텍스트 상자·시각적 배치를 대표 파일로 검증 |
...
5. **대표 문서군으로 정책을 검증한다.** PDF는 라벨·OCR·읽기 순서, DOCX는 스타일·번호 체계, PPTX는 자리표시자·텍스트 상자·슬라이드 그룹을 각자 측정한다.

---

## 마무리 문장

> Docling의 가치는 단순 텍스트 추출이 아니라, 문서 항목과 위치·관계를 보존해 후속 구조화와 검수를 가능하게 하는 데 있다. 공통 파이프라인은 원본 참조와 읽기 순서를 지키고, 형식별 차이는 관찰된 구조 신호에 맞춰 작게 보강한다.
