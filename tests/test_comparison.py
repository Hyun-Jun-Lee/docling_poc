...
def test_report_preserves_run_numbers_and_escapes_document_content(tmp_path, schema_version):
    ...
    assert '#### Markdown 원문' not in markdown
    assert '#### JSON 원본' not in markdown
    ...
    assert '원본 JSON을 읽을 수 없습니다' not in markdown
    ...
    assert html.index('대용량 처리 운영 확장성') > html.index('markdown-d1-docling')
    assert '추출 정보 비교' in html
    assert '기존 구조 검토 도구 펼치기' not in html
    assert 'structure-review' not in html
    assert 'structure_review' not in result['documents'][0]
    assert set(result['documents'][0]['feature_comparison']) == {'docling', 'tika'}
